...
export default function DynamicFintechNavbar({ onOpenAuth, brandCount = 6 }: NavbarProps) {
  ...
  // 80px scroll hone par hi navbar screen par aayega
  useMotionValueEvent(scrollY, "change", (latest) => {
    if (latest > 80) {
      setShowNavbar(true);
    } else {
      setShowNavbar(false);
      setMobileMenuOpen(false); // agar top par chale jayein toh mobile menu bhi band ho jaye
    }
  });
    ...
    <>
      <AnimatePresence>
        {showNavbar && (
          ...
          >
            <nav className="pointer-events-auto w-full max-w-4xl py-2 px-4 sm:px-6 rounded-full bg-[#08090C]/85 border border-white/[0.12] backdrop-blur-2xl shadow-[0_20px_50px_rgba(0,0,0,0.8),0_0_30px_rgba(16,185,129,0.06)] flex items-center justify-between transition-all duration-300">
              
              {/* 1. Brand Floating Logo & Title */}
      ...
      {/* 4. Mobile Drawer Menu (Visible only when Navbar is active & toggled) */}
      <AnimatePresence>
